...
def decompose(file, channel, length, feature, custom=False):
	...
	if length == 3: decomposed_de = np.empty([0,8,20])
	elif length == 1: decomposed_de = np.empty([0,8,60])
	else: raise Exception("Length must be either 1 or 3 sec.")

	if channel == 5: base_DE = np.empty([0,40])
	elif channel == 32: base_DE = np.empty([0,256])	
	...
	for trial in range(40):
		...
		if length == 3: temp_de = np.empty([0,40])
		elif length == 1: temp_de = np.empty([0,120])
		...
		for chan in channels:			
			...
			base_theta_DWT, base_alpha_DWT, base_beta_DWT, base_gamma_DWT = dwt_fn_baseline(base_signal, "entropy", length)
			...
			temp_base_theta_DE = np.append(temp_base_theta_DE,base_theta_DWT)
			temp_base_gamma_DE = np.append(temp_base_gamma_DE,base_gamma_DWT)
			temp_base_beta_DE = np.append(temp_base_beta_DE,base_beta_DWT)
			temp_base_alpha_DE = np.append(temp_base_alpha_DE,base_alpha_DWT)
			...
			for index in range(60):
				if length == 1:
					theta_de, alpha_de, beta_de, gamma_de = de_fn(trial_signal[index*frequency:(index+1)*frequency])
					theta_dwt, alpha_dwt, beta_dwt, gamma_dwt = dwt_fn(trial_signal[index*frequency:(index+1)*frequency], feature)
					
					DE_theta =np.append(DE_theta,theta_de)
					DE_alpha =np.append(DE_alpha,alpha_de)
					DE_beta =np.append(DE_beta,beta_de)
					DE_gamma =np.append(DE_gamma,gamma_de)
					...
					if index == 0 or index % 3 == 0: 
						theta_de, alpha_de, beta_de, gamma_de = de_fn(trial_signal[index*frequency:(index+3)*frequency])
						theta_dwt, alpha_dwt, beta_dwt, gamma_dwt = dwt_fn(trial_signal[index*frequency:(index+3)*frequency], feature)
		...
		if length == 3: temp_trial_de = temp_de.reshape(-1,8,20)
		if length == 1: temp_trial_de = temp_de.reshape(-1,8,60)
	...
	decomposed_de = decomposed_de.reshape(-1,channel,8,int(60/length)).transpose([0,3,2,1]).reshape(-1,8,channel).reshape(-1,channel*8)
...
if __name__ == '__main__':	
	...
	if custom: result_dir = str(os.getcwd())+f"/1D_dataset_stacked_custom/{channel_dir}chan/{length_dir}sec/"
	else: result_dir = str(os.getcwd())+f"/1D_dataset_stacked/{channel_dir}chan/{length_dir}sec/"
	...
	for file in os.listdir(dataset_dir):
		...
		if os.path.exists(result_dir+"DE_"+file):
			...
			continue
		base_DE,trial_DE = decompose(file_path, int(channel_dir), int(length_dir), "entropy", custom)
...
#python.exe get_1D_data.py 5 1 True