...
def get_dataset_deviation(trial_data,base_data, channel, length):
	if channel == 32:
		new_dataset = np.empty([0,256])
	elif channel == 5:
		new_dataset = np.empty([0,40])
	...
	for i in range(0,length_trial):
		...
		if channel == 32: new_record = get_vector_deviation(trial_data[i],base_data[base_index]).reshape(1,256)
		elif channel == 5: new_record = get_vector_deviation(trial_data[i],base_data[base_index]).reshape(1,40)
...
def pre_process(path,y_n, channel, length):
	...
	if channel == 32: data_3D = data_3D.reshape(-1,8,9,9)
	elif channel == 5: data_3D = data_3D.reshape(-1,8,5,5)
...
if __name__ == '__main__':
	...
	custom = False
	dataset_dir = cwd+f"/1D_dataset_stacked/{channel_dir}chan/{length_dir}sec/"
	if len(sys.argv) == 5:		
		custom = True
		dataset_dir = cwd+f"/1D_dataset_stacked_custom/{channel_dir}chan/{length_dir}sec/"
	...
	if use_baseline=="yes":
		result_dir = cwd+f"/3D_dataset_stacked{custom_dir}/{channel_dir}chan/{length_dir}sec/with_base/"
		if os.path.isdir(result_dir)==False:
			os.makedirs(result_dir)
	else:
		result_dir = cwd+f"/3D_dataset_stacked{custom_dir}/{channel_dir}chan/{length_dir}sec/without_base/"
		if os.path.isdir(result_dir)==False:
			os.makedirs(result_dir)
...
#python.exe 1D_to_3D.py yes 5 1 True