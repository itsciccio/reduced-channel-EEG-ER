...
inputs = list(map(int,[1,2,3,4,5,6,7,8]))
...
dataset_dir = cwd+f"/3D_dataset_stacked{custom_dir}/{channel_dir}chan/{length_dir}sec/"+with_or_not+"_base/DE_"
...
if input_file == "independent":
    #load all files
    combined_data = np.empty([0,8,input_height,input_width])
...
for subject_file in all_subs:
    ...
    #     train_y  = train_y[index]
        ...
        #                            'f1_score': test_f1})  # , 'roc_auc':test_auc})
            ...
            if input_file != "independent": dir_path = cwd+f"/result_stacked{custom_dir}/{channel_dir}chan/{length_dir}sec/"+with_or_not+f"/{kernel_size}x{kernel_size}/"+arousal_or_valence+"/"
            else: dir_path = cwd+f"/result_stacked{custom_dir}/{channel_dir}chan/{length_dir}sec/"+with_or_not+f"/{kernel_size}x{kernel_size}/"+arousal_or_valence+"/independent/"
