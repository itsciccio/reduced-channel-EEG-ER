...
#dir_path = cwd+"/result_entropy/5chan/3sec/"+with_or_not+"/"+file_dir+f"/{kernel_height_1st}x{kernel_width_1st}/"+arousal_or_valence+"/"
...
inputs = list(map(int,[1,2,3,4]))
bands = list(map(minus,inputs))
print(bands)

if int(channel_dir) == 32:    
    input_height = 9
    input_width = 9
else:     
    input_height = 5
...
dataset_dir = cwd+f"/3D_dataset_{feature_type}{custom_dir}/{channel_dir}chan/{length_dir}sec/"+with_or_not+"_base/DE_"

all_subs = ["s01", "s02", "s03", "s04", "s05", "s06", "s07", "s08", "s09", "s10", "s11", "s12", "s13", "s14", "s15", "s16", "s17", "s18", "s19", "s20", "s21", "s22", "s23", "s24", "s25", "s26", "s27", "s28", "s29", "s30", "s31", "s32"]
if "s" in input_file:
    all_subs = [input_file]

if input_file == "independent":
    #load all files
    combined_data = np.empty([0,4,input_height,input_width])
    #combined_labels = np.empty([0])
    for subject_file in all_subs:
        print("loading ",dataset_dir+subject_file,".mat")
        data_file = sio.loadmat(dataset_dir+subject_file+".mat")
        combined_data = np.vstack([combined_data, data_file['data']])

        if subject_file == "s01": 
            combined_labels = data_file[f"{arousal_or_valence}_labels"][0,:]
            original_data_shape = data_file['data'].shape
            original_data_labels = data_file[f"{arousal_or_valence}_labels"][0,:].shape
        else: combined_labels = np.concatenate([combined_labels, data_file[f"{arousal_or_valence}_labels"][0,:]])
    
    print("Original Data Shape:")
    print(original_data_shape)
    print(original_data_labels)
    print("\nCombined Data Shape:")
    print(combined_data.shape)
    print(combined_labels.shape)
    print()

    sio.savemat(dataset_dir+"combined_data.mat",{"data":combined_data,f"{arousal_or_valence}_labels":combined_labels})
    

    all_subs = ["combined_data"]
...
for subject_file in all_subs:
    ...
    if all_subs == ["combined_data"]: training_epochs = 20
    else: training_epochs = 50

    batch_size = 128

    # kernel parameter

    kernel_height_1st = int(kernel_size)
    kernel_width_1st = int(kernel_size)

    kernel_height_2nd = int(kernel_size)
    kernel_width_2nd = int(kernel_size)

    kernel_height_3rd = int(kernel_size)
    kernel_width_3rd = int(kernel_size)
    ...
    #         temp_index = [j for j in range(i*20,i*20+10)]
    #         test_index = np.append(test_index,temp_index)

    #     fine_tune_index = np.setxor1d([i for i in range(0,800)],test_index)
    ...
    for curr_fold, (train_index, test_index) in enumerate(skf.split(cnn_datasets, labels.argmax(1))):

        config = tf.ConfigProto()
        config.gpu_options.allow_growth = True

        cnn_train_x, cnn_test_x = cnn_datasets[train_index], cnn_datasets[test_index]
    ...
    #     train_y  = train_y[index]
        ...
        with tf.Session(config=config) as session:
            ...
            for epoch in range(training_epochs):
                ...
                if (epoch % 1 == 0):
                    ...
                    for i in range(train_accuracy_batch_num):
                        ...
                        if input_file == "independent":
                            if i == train_accuracy_batch_num -1:
                                continue
                    ...
                    for j in range(test_accuracy_batch_num):
                        ...
                        if input_file == "independent":
                            if j == test_accuracy_batch_num -1:
                                continue
            ...
            for k in range(test_accuracy_batch_num):
                ...
                if input_file == "independent":
                    if k == test_accuracy_batch_num -1:
                        continue
        ...
        #                            'f1_score': test_f1})  # , 'roc_auc':test_auc})
            ...
            if input_file != "independent": dir_path = cwd+f"/result_{feature_type}{custom_dir}/{channel_dir}chan/{length_dir}sec/"+with_or_not+f"/{kernel_size}x{kernel_size}/"+arousal_or_valence+"/"
            else: dir_path = cwd+f"/result_{feature_type}{custom_dir}/{channel_dir}chan/{length_dir}sec/"+with_or_not+f"/{kernel_size}x{kernel_size}/"+arousal_or_valence+"/independent/"

            if os.path.isdir(dir_path)==False:
                os.makedirs(dir_path)            

            file_path = dir_path+subject_file+"_"+str(curr_fold)+".xlsx"

            if os.path.exists(dir_path+subject_file+"_9.xlsx"):
                exit(0)
...
#python cnn.py 5 3 with 4 arousal s01 entropy
#python cnn.py 5 3 with 4 arousal all entropy
#python cnn.py 5 1 with 4 arousal all de 
#python cnn.py 5 1 with 4 arousal independent de 